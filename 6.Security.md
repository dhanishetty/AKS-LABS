...
#### 4. The commands that build the trust chain

The trust chain above is set up with two commands — one on the **Kubernetes side** and one on the **Azure side**. Both must reference the *same* namespace, service account name, and managed identity for the exchange to work.

##### Step 1 — Create the Kubernetes ServiceAccount (`kubectl`)
...
- **`kubectl apply -f -`** — applies a manifest read from **stdin** (the `-` means "standard input" instead of a file).
- **`<<EOF ... EOF`** — a shell **here-document**. Everything between `<<EOF` and the closing `EOF` is piped into `kubectl` as YAML. Because the heredoc is unquoted, the shell **expands the `${...}` variables first**, so the real values are substituted before `kubectl` ever sees the manifest.
- **`kind: ServiceAccount`** — creates the in-cluster identity.
- **`name` / `namespace`** — where the SA is created (name + namespace together uniquely identify it).
- **`annotations: azure.workload.identity/client-id`** — links this SA to a **user-assigned managed identity** by its Client ID. This is the Kubernetes side saying: *"This ServiceAccount wants to act as managed identity X."*

##### Step 2 — Create the federated identity credential (`az`)
...
This adds a **federated identity credential** to the managed identity — a rule that says *"accept an external OIDC token instead of a secret, but only if it matches these exact conditions"*:

| Flag | What it does |
| --- | --- |
| `--name` | Friendly label for this credential rule. |
| `--identity-name` | The **user-assigned managed identity** the credential is attached to (the identity the pod will act as). |
| `--resource-group` | Resource group containing that managed identity. |
| `--issuer` | Your **AKS cluster's OIDC Issuer URL**. Entra ID uses it to fetch the cluster's public keys and verify the token was minted by your cluster. |
| `--subject` | The exact **`sub` claim** the token must carry — `system:serviceaccount:<namespace>:<name>`. Locks trust to one specific SA in one specific namespace. |
| `--audience` | The **`aud` claim** the token must carry. `api://AzureADTokenExchange` is the fixed, standard value for Microsoft Entra Workload Identity. |

Entra ID hands back an Azure token **only if issuer + subject + audience all match**. A pod in a different namespace, or from a different cluster, is rejected.

##### How the two commands pair up

```
Kubernetes side (kubectl)                    Azure side (az identity federated-credential)
─────────────────────────                    ─────────────────────────────────────────────
ServiceAccount annotated with                Federated credential on the managed identity
azure.workload.identity/client-id    ◄────►  matching issuer + subject + audience
= the managed identity's Client ID           = "trust tokens from THIS SA in THIS namespace"
```

- **Step 1 (kubectl):** "This ServiceAccount wants to *be* managed identity X."
- **Step 2 (az):** "Managed identity X *agrees to trust* tokens from that exact ServiceAccount / namespace / cluster."

Both sides must agree — matching subject (`namespace:serviceaccount`), issuer (cluster OIDC URL), and client ID — for a pod to obtain an Azure token with **zero stored secrets**.
...
#### 5. Granting the identity access to Key Vault (RBAC authorization)
...
Steps 1–2 handled **authentication** — proving the pod *is* the managed identity. But authentication alone grants **zero** access. This command handles **authorization**: it uses **Azure RBAC** to give that identity permission to actually read secrets from a specific Key Vault.

| Flag | What it does |
| --- | --- |
| `--assignee-object-id` | The **Principal (Object) ID** of the managed identity's service principal — the identity that *receives* the permission. Note this is the **Object/Principal ID, not the Client ID** used in the ServiceAccount annotation (steps 1–2). Same identity, different ID. |
| `--role "Key Vault Secrets User"` | The **built-in RBAC role** being granted. It allows reading/getting secret *values* (the data plane) — but not managing the vault itself. |
| `--scope "${AKV_ID}"` | The resource the permission applies to — the specific **Key Vault's resource ID**. Least privilege: access is limited to just this one vault, not the whole resource group or subscription. |
| `--assignee-principal-type ServicePrincipal` | Tells Azure the assignee is a **service principal**. (Recall: *every managed identity is backed by a service principal*.) Setting this explicitly skips a Microsoft Graph lookup and avoids "principal not found" errors caused by replication delay for a freshly created identity. |

##### How it connects to the ongoing concept

This is the **AuthZ** half of the `AuthN` vs `AuthZ` split from the [IAM fundamentals](#what-is-identity-and-access-management) section:

- **Authentication (steps 1–2):** The pod exchanges its OIDC token for an Azure access token — Entra ID confirms *who* it is.
- **Authorization (step 5):** RBAC decides *what* that identity is allowed to do.

Back in the **hotel analogy**, steps 1–2 verified your identity at the front desk and handed you a keycard (token). This step is the hotel **programming that keycard to open one specific room** (the Key Vault) — and nothing else. Without it, the pod gets a valid Azure token but every attempt to read a secret returns **403 Forbidden**.

The complete chain end-to-end:

```
Pod (SA) ──token exchange──► Microsoft Entra ID ──access token──► Pod ──"read secret"──► Key Vault
   \_____ steps 1–2: authentication _____/                          \__ step 5: RBAC authorization __/
```

---

## The Big Picture — Connecting All the Concepts

The diagram below ties together everything in this document: the **IAM foundations** (AuthN vs AuthZ, OAuth roles, tokens), **Microsoft Entra ID** identities (managed identity = service principal), the **AKS OIDC issuer**, the **ServiceAccount + namespace**, the **federated credential**, and the final **RBAC** authorization — all working so a pod reads a secret with **zero stored credentials**.

```mermaid
flowchart TD
    subgraph FOUND["Foundations — IAM"]
        AUTHN["AuthN = OpenID Connect<br/>proves WHO you are"]
        AUTHZ["AuthZ = OAuth 2.0 / Azure RBAC<br/>controls WHAT you can do"]
    end

    subgraph CLUSTER["AKS Cluster — OIDC Issuer / Identity Provider"]
        NS["Namespace<br/>e.g. production"]
        SA["ServiceAccount<br/>annotation: azure.workload.identity/client-id"]
        POD["Pod = OAuth Client<br/>the workload"]
        OIDCURL["OIDC Issuer URL<br/>public signing keys"]
        NS --> SA
        SA --> POD
    end

    subgraph ENTRA["Microsoft Entra ID — Authorization Server"]
        MI["User-assigned Managed Identity<br/>backed by a Service Principal"]
        FIC["Federated Identity Credential<br/>issuer + subject + audience"]
        RBAC["Role Assignment<br/>Key Vault Secrets User at vault scope"]
        MI --> FIC
        MI --> RBAC
    end

    KV["Azure Key Vault — Resource Server"]

    POD -->|"1 - projected JWT, sub=system:serviceaccount:ns:sa"| ENTRA
    FIC -.->|"2 - verify token signature via"| OIDCURL
    ENTRA -->|"3 - Azure access token"| POD
    POD -->|"4 - request secret + bearer token"| KV
    KV -.->|"5 - check RBAC permissions"| RBAC
    KV -->|"6 - secret returned"| POD

    AUTHN -.->|governs steps 1-3| ENTRA
    AUTHZ -.->|governs steps 5-6| RBAC
```

**How to read it, mapped to the concepts:**

| Element in the diagram | Concept from this document |
| --- | --- |
| **Pod** | The **OAuth Client** / workload identity requesting access |
| **Microsoft Entra ID** | The **Authorization Server** and **Identity Provider (IdP)** |
| **Azure Key Vault** | The **Resource Server** holding the protected data |
| **Managed Identity** | A **Service Principal** (every managed identity is one) the pod acts as |
| **OIDC Issuer URL** | The cluster acting as its own **IdP**, letting Entra verify the token |
| **ServiceAccount + Namespace** | The Kubernetes identity; `sub` claim = `system:serviceaccount:ns:sa` |
| **Federated Identity Credential** | The **trust rule** matching issuer + subject + audience |
| **Role Assignment (RBAC)** | **Authorization** — what the identity is allowed to do |
| **Steps 1–3** | **Authentication (AuthN)** via OpenID Connect token exchange |
| **Steps 4–6** | **Authorization (AuthZ)** via Azure RBAC |
