...
## OpenID Connect and Workload Identity on AKS
...
### Service Account & Service Account Namespace
...
If someone tries to deploy a malicious pod in the `development` namespace using a Service Account with the same name, Azure Entra ID will look at the incoming token, see the namespace mismatch, and **reject the login attempt**.
...
3. **The Pod** sends this token to Azure Entra ID, asking to authenticate as the Azure Managed Identity (**ClientID**).
4. **Azure Entra ID** checks its federated credential rules. It asks:
