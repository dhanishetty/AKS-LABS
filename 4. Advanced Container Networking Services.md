...
| Component                                         | Improved Definition (AKS Context)                                                                                                                                                                                                                                                          |
| :------------------------------------------------ | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **CNI (Container Network Interface)**             | The Kubernetes networking framework responsible for assigning IP addresses to pods and connecting them to the network. In AKS, common options include **Azure CNI Overlay** (pods use an internal overlay network) and **Azure CNI Flat** (pods receive IPs directly from the Azure VNet). |
| **Cilium**                                        | An **eBPF-based CNI and security platform** that replaces traditional iptables rules with kernel-level packet processing. It provides networking, network policies, load balancing, service discovery, and observability with lower overhead and better scalability.                       |
| **ACNS (Advanced Container Networking Services)** | Azure's managed integration of Cilium that adds enterprise features such as **network policies, flow logs, network metrics, Hubble observability, and FQDN filtering**, all managed through native AKS and Azure tooling.                                                                  |
| **Container Network Flow Logs**                   | A feature of ACNS that captures **individual network flow records** such as *Source IP, destination IP, ports, protocol (TCP/UDP/ICMP), bytes, packets, and whether traffic was allowed or dropped* and sends them to Azure Log Analytics for troubleshooting, auditing, and forensic analysis. Who talked to whom? Was it allowed or denied?                                                  |
| **Container Network Metrics**                     | A feature of ACNS that converts raw network activity into **Prometheus-style time-series metrics**, such as *packet drops, DNS latency or HTTP request statistics when application-layer visibility is enabled, throughput, and connection counts*, enabling dashboards and alerting. How much traffic? How many drops? What is the latency?                                                                               |
| **Hubble**                                        | Cilium's observability layer that uses eBPF events to provide **real-time visibility into pod-to-pod, pod-to-service, and external communications**, including DNS requests, HTTP traffic, and policy decisions.                                                                           |
| **Prometheus**                                    | The time-series monitoring system that **scrapes and stores metrics** from Kubernetes components, applications, and ACNS/Cilium. In AKS, this is commonly provided through **Azure Managed Prometheus**.                                                                                   |
| **Grafana**                                       | The dashboard and visualization platform used to create **graphs, heatmaps, service maps, and alerts** from Prometheus metrics. AKS commonly integrates with **Azure Managed Grafana**.                                                                                                    |
| **Log Analytics Workspace**                       | Azure's centralized log storage service that collects and retains **textual and event-based data**, including container logs, flow logs, diagnostics, and audit information, which can be queried using **Kusto Query Language (KQL)**.                                                    |
