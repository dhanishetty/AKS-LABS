...
- Click on `Edit` (top right corner of webpage) → `Add` (New option appears beside edit) and then `Visualization`
...
| ---------------- | --------------------- | --------------------------------------------------------------------------------------------------------------------- | ----------------- |
| `PodMonitor`     | Pods (via labels)     | Scrape metrics directly from pods (e.g., sidecar exporters or standalone application pods)                     | No                |
...
| Term                    | Meaning                                                                     |
| ----------------------- | --------------------------------------------------------------------------- |
| `AMA_METRICS_POD_NAME=` | Stores the command output in a shell variable named `AMA_METRICS_POD_NAME`. |
| `$(...)`                | Command substitution; executes the command and returns its output.          |
| `kubectl get po`        | Gets Kubernetes pods (`po` is short for `pods`).                            |
| `-n kube-system`        | Searches in the `kube-system` namespace.                                    |
| `-l rsName=ama-metrics` | Filters pods with the label `rsName=ama-metrics`. A label key, typically indicating the name of the ReplicaSet (rs = ReplicaSet) that created or manages the pod.                           |
| `-o jsonpath='...'`     | Formats the output using a JSONPath expression.                             |
| `{.items[0]}`           | Selects the first pod in the result list.                                   |
| `.metadata.name`        | Retrieves the pod's name.                                                   |
...
In the Promethuus website top left → click on `status` → `Targets` → you'll find the target: `podMonitor/default/prometheus-reference-app-job/0`
...
### Inspecting the Raw Metrics directly from the Reference Application
...
* Open your browser at: http://localhost:2112/metrics
* Or use curl to fetch the metrics:
